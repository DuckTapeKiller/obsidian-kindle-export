...
var KindleSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Author").setDesc("Default author for new documents").addText(
      (text) => text.setPlaceholder("Obsidian").setValue(this.plugin.settings.author).onChange(async (value) => {
        this.plugin.settings.author = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Email").setDesc("Approved Email for your Kindle").addText(
      (text) => text.setPlaceholder("you@obsidian.md").setValue(this.plugin.settings.sendmail).onChange(async (value) => {
        this.plugin.settings.sendmail = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Kindlemail").setDesc("Your Kindle/PocketBook email").addText(
      (text) => text.setPlaceholder("you@kindle.com").setValue(this.plugin.settings.kindlemail).onChange(async (value) => {
        this.plugin.settings.kindlemail = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("SMTP Host").setDesc("Your SMTP host (e.g. smtp.gmail.com)").addText(
      (text) => text.setPlaceholder("smtp.obsidian.md").setValue(this.plugin.settings.smtphost).onChange(async (value) => {
        this.plugin.settings.smtphost = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("SMTP Port").setDesc("Your SMTP port (e.g. 587)").addText(
      (text) => text.setPlaceholder("465").setValue(this.plugin.settings.port).onChange(async (value) => {
        this.plugin.settings.port = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("SMTP User").setDesc("Username for your SMTP server e.g. your Mailadress").addText(
      (text) => text.setPlaceholder("you@obsidian.md").setValue(this.plugin.settings.user).onChange(async (value) => {
        this.plugin.settings.user = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("SMTP Password").setDesc("Your SMTP password").addText(
      (text) => text.setPlaceholder("********").setValue(this.plugin.settings.pass).onChange(async (value) => {
        this.plugin.settings.pass = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Backend").setDesc("Your backendadress (e.g. https://staneks.de/apps/md2mobi/) can be used for free or host your own Backend.").addText(
      (text) => text.setPlaceholder("https://ob2ki.com").setValue(this.plugin.settings.backend).onChange(async (value) => {
        this.plugin.settings.backend = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Generate TOC").setDesc("Generate Table of Contents.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.toc).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Pagebreak by '---'").setDesc("Suggested feature: Activate pagebreak by '---'").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.pagebreak).onChange(async (value) => {
        this.plugin.settings.pagebreak = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Markdown merge").setDesc("Suggested feature: Merge .md files into one file (adds Kindle: Mergedown command) Plugin reloads after saving.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.mergedown).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Exportpath").setDesc("Set the path where you want to export your files to. (e.g. /folder)").addText(
      (text) => text.setPlaceholder("/folder").setValue(this.plugin.settings.expath).onChange(async (value) => {
        this.plugin.settings.expath = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Add Ribbon-Icon").setDesc("Suggested feature: Add Ribbon-Icon for the Export-Command. Plugin reloads after saving.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.ribbonicon).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Bulk Export Folder").setDesc("Folder to export all files from (e.g. '2 - ART\xCDCULOS/PEDRO PEREZ').").addText(
      (text) => text.setPlaceholder("Folder/Path").setValue(this.plugin.settings.bulkFolder).onChange(async (value) => {
        this.plugin.settings.bulkFolder = value;
        await this.plugin.saveSettings();
        console.log(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Bulk Export Mode").setDesc("Enable to export all files in the Bulk Export Folder. DISABLE THIS for normal single-file export!").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.bulkMode).onChange(async (value) => {
...
var Kindle = class extends import_obsidian2.Plugin {
  ...
  async GetEbook(lines, Inhalt, imagelist, imagename, links) {
        ...
        if (file.extension == "md") {
          ...
          this.app.metadataCache.iterateReferences((sourcePath, reference) => {
            if (reference.link.contains(file.basename) && reference.original == FileLink && Iteration == 0) {
              ...
              if (ankerName != void 0 && ankerSplit > 1) {
  ...
  async export() {
    if (this.settings.bulkMode) {
      ...
      if (!folderPath) {
        new import_obsidian2.Notice("Please set a Bulk Export Folder in settings!");
        return;
      }
      ...
      if (targetFiles.length === 0) {
        new import_obsidian2.Notice(`No Markdown files found in folder: ${folderPath}`);
        return;
      }
      new import_obsidian2.Notice(`Starting Bulk Export of ${targetFiles.length} files from '${folderPath}'...`);
      for (let i = 0; i < targetFiles.length; i++) {
        let file = targetFiles[i];
        try {
          if (i % 5 === 0 && i > 0)
            new import_obsidian2.Notice(`Exporting ${i}/${targetFiles.length}...`);
          await this.processDocument(file);
          await new Promise((r) => setTimeout(r, 1e3));
        } catch (e) {
          console.error(`Failed to export ${file.name}`, e);
          new import_obsidian2.Notice(`Failed to export ${file.name}`);
        }
      }
      new import_obsidian2.Notice("Bulk Export Complete!");
    } else {
  ...
  async processDocument(dokument) {
    let Inhalt = "";
    let imagelist = [];
    let imagename = [];
    let links = [];
    ...
    if (data.startsWith("---")) {
      ...
      let lines2 = frontmatterRaw.split("\n");
    ...
    let author = extractedAuthor ? extractedAuthor : this.settings.author;
  ...
  async Mergedown(lines, Inhalt, imagelist, imagename, links) {
    for (let i = 0; i < lines.length; i++) {
      let text = lines[i];
      if (text.contains("![[") && text.contains("]]") || text.contains("![") && text.contains(")") && text.contains("http://") && text.contains("https://")) {
        let fileArray = await this.getFile(text, links);
